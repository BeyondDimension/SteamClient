...
* https://github.com/BeyondDimension/original.winauth/blob/master/Authenticator/Authenticator.cs
*/
...
using static WinAuth.Abstractions.IAuthenticatorValueModel;
...
public partial class AuthenticatorValueModel : IAuthenticatorValueModel
{
    ...
    /// 默认期限为 30s
    /// </summary>
    public const int DEFAULT_PERIOD = AuthenticatorExportModel.DEFAULT_PERIOD;

    public const HMACTypes DEFAULT_HMACTYPE = AuthenticatorExportModel.DEFAULT_HMAC_TYPE;
    ...
    public const HMACTypes DEFAULT_HMAC_TYPE = AuthenticatorExportModel.DEFAULT_HMAC_TYPE;
    ...
    protected static void AppendHexStringToUTF8Bytes(StringBuilder b, string? s)
    {
        if (!string.IsNullOrEmpty(s))
        {
            b.Append(Convert.ToHexString(Encoding.UTF8.GetBytes(s)));
        }
    }

    protected virtual void WriteSecretData(StringBuilder b)
    {
        #region OLD
        ...
        b.Append(CodeDigits);
        b.Append('\t');
        b.Append(HMACType.ToString());
        b.Append('\t');
        b.Append(Period);
    }

    protected virtual void SetSecretData(ReadOnlySpan<char> s)
    {
        #region OLD
        ...
        if (parts.MoveNext())
        {
            ...
            if (r1.HasValue)
            {
                if (int.TryParse(s[r1.Value], out var digits))
                {
                    CodeDigits = digits;
                }
            }
            if (r2.HasValue)
            {
                HMACType = Enum.Parse<HMACTypes>(s[r2.Value]);
            }
            if (r3.HasValue)
            {
                if (int.TryParse(s[r3.Value], out var period) == true)
                {
                    Period = period;
                }
            }
    ...
    public void SetEncryption(PasswordTypes passwordType, string? password = null)
    {
        ...
        {
            using var ms = AuthenticatorModel.Manager.GetStream();
            ...
            var data = Convert.ToHexString(ms.GetSpan());
    ...
    public bool Unprotect(string? password)
    {
        ...
        try
        {
            ...
            using (var ms = AuthenticatorModel.Manager.GetStream(Convert.FromHexString(data)))
            {
            ...
            if (changed == true)
            {
                ...
                var encrypteddata = Convert.ToHexString(ms.GetSpan());
    ...
    */
    ...
    static void ThrowIsNotDone(OperationStatus statusFromHexString)
    {
        if (statusFromHexString != OperationStatus.Done)
            throw new ArgumentOutOfRangeException(nameof(statusFromHexString), statusFromHexString, null);
    }
    ...
    public static string DecryptSequence(string data, PasswordTypes encryptedTypes, string? password, bool decode = false)
    {
        ...
        var sha = SafeHasher2("SHA256");
        const int sha256_hashSize = 256;
        {
            // jump header
            var datastart = ENCRYPTION_HEADER.Length;
            var salt = data.AsSpan(datastart, Math.Min(SALT_LENGTH * 2, data.Length - datastart));
            datastart += salt.Length;
            var hash = data.AsSpan(datastart, Math.Min(/*sha.HashSize*/ sha256_hashSize / 8 * 2, data.Length - datastart));
            ...
            try
            {
                ...
                if (hash.Equals(comparehash, StringComparison.OrdinalIgnoreCase))
                {
            ...
            {
                ArrayPool<char>.Shared.Return(chars);
                ArrayPool<byte>.Shared.Return(compareplain);
            }
        }
    ...
    static string DecryptSequenceNoHash(string data, PasswordTypes encryptedTypes, string? password, bool decode = false)
    {
        try
        {
            ...
            if ((encryptedTypes & PasswordTypes.Machine) != 0)
            {
                ...
                var cipher = Convert.FromHexString(data);
                ...
                {
                    data = Convert.ToHexString(plain);
                }
            }
            ...
            if ((encryptedTypes & PasswordTypes.Explicit) != 0)
            {
                ...
                if (decode == true)
                {
                    var plain = Convert.FromHexString(data);
                    data = Encoding.UTF8.GetString(plain, 0, plain.Length);
                }
            }
        ...
        {
            throw new BadPasswordException(ex);
        }
    ...
    /// </summary>
    public static Func<ReadOnlySpan<byte>, byte[]> SafeHasher2(string name)
    {
        if (name == "SHA512")
        {
    ...
    public static string EncryptSequence(string data, PasswordTypes passwordType, string? password)
    {
        ...
        var sha = SafeHasher2("SHA256");
        {
            var plain = Convert.FromHexString(salt + data);
            hash = Convert.ToHexString(sha(plain));
        }
        ...
        if ((passwordType & PasswordTypes.User) != 0)
        {
            ...
            data = Convert.ToHexString(cipher);
        }
        if ((passwordType & PasswordTypes.Machine) != 0)
        {
...
#else
        var localDataProtectionProviderService = Ioc.Get<global::BD.Common8.Security.Services.ILocalDataProtectionProvider.IProtectedData>();
    ...
    {
        var inBytes = Convert.FromHexString(plain);
...
#pragma warning restore SA1313 // Parameter names should begin with lower-case letter
    {
        byte[] key;
        var saltBytes = Convert.FromHexString(data.AsSpan()[..(SALT_LENGTH * 2)]);
        ...
        var inBytes = Convert.FromHexString(data.AsSpan()[(SALT_LENGTH * 2)..]);
    ...
    public static string Decrypt(string data, byte[] key)
    {
        ...
        var inBytes = Convert.FromHexString(data);
