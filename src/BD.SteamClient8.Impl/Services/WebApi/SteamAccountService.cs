...
using R = BD.SteamClient8.Resources.Strings;
        ...
        serviceProvider), ISteamAccountService
{
    ...
    /// <inheritdoc/>
    public async Task<(string encryptedPassword64, ulong timestamp)> GetRSAkeyV2Async(string username, string password, CancellationToken cancellationToken = default)
    {
        ...
        var requestUriString = string.Format(SteamApiUrls.GetRSAkeyV2Url, input_protobuf_encoded);
        ...
        bool isSteamClientPlatform = false)
    {
        ...
        var client = CreateClient(username);
        var cookieContainer = GetCookieContainer(username);
        ...
        else
        {
            ...
            {
                return loginState.Message = R.请填写正确的Steam用户名密码;
            }
            ...
            if (string.IsNullOrEmpty(loginState.SeesionId))
            {
                ...
                {
                    await client.GetAsync(SteamApiUrls.STEAM_STORE_LOGIN_URL, cancellationToken);
                });
                loginState.SeesionId = cookieContainer.GetCookieValue(new Uri(SteamApiUrls.STEAM_STORE_URL, UriKind.Absolute), "sessionid");
            }

            var getRSAKeyResult = await GetRSAkeyV2Async(loginState.Username, loginState.Password, cancellationToken);
            var (encryptedPassword64, timestamp) = getRSAKeyResult;

            if (string.IsNullOrEmpty(encryptedPassword64))
            {
                loginState.ResetStatus();
                return loginState.Message = R.登录失败获取RSAkey出现错误请重试;
            }
            ...
            if (loginState.SteamId == 0)
            {
                ...
                {
                    "5" => R.请核对您的密码和账户名称并重试,
                    "20" => R.与Steam通信时出现问题请稍后重试,
                    "84" => R.短期内来自您所在位置的失败登录过多请15分钟后再试,
                    _ => R.登录遇到未知错误请稍后重试_.Format(eResult),
                };
            ...
            if (result.AllowedConfirmations.Count > 0)
            {
                if (result.AllowedConfirmations[0].ConfirmationType == EAuthSessionGuardType.KEauthSessionGuardTypeDeviceCode)
                {
                    ...
                    loginState.Success = false;
                    return loginState.Message = R.需要2FA验证码;
                }
                else if (result.AllowedConfirmations[0].ConfirmationType == EAuthSessionGuardType.KEauthSessionGuardTypeEmailCode)
                {
                    ...
                    return loginState.Message = R.需要邮箱验证码;
                }
            }
        }
        ...
        {
            loginState.ResetStatus();
            return loginState.Message = R.登录失败请确认输入验证码是否正确;
        }
        ...
        {
            loginState.ResetStatus();
            return loginState.Message = R.FinalizeLoginAsync登录失败;
        }
        ...
        if (tokens.TransferInfo?.Count > 0)
        {
            foreach (var transfer in tokens.TransferInfo)
            {
                if (string.IsNullOrEmpty(transfer.Url) ||
                    transfer.Url?.Contains("help.steampowered.com") == true ||
        ...
        async Task<(CAuthentication_BeginAuthSessionViaCredentials_Response, HttpResponseMessage)> BeginAuthSessionViaCredentials(string encryptedPassword64, ulong timestamp, CancellationToken cancellationToken = default)
        {
            ...
            return await WaitAndRetryAsync(sleepDurations).ExecuteAsync(async () =>
            {
                ...
                using var responseStream = await response.ThrowIsNull().Content.ReadAsStreamAsync(cancellationToken);
        ...
        {
            var loginStateClientId = loginState.ClientId;
            if (!loginStateClientId.HasValue)
#pragma warning disable CA2208 // 正确实例化参数异常
                throw new ArgumentNullException(nameof(loginStateClientId));
#pragma warning restore CA2208 // 正确实例化参数异常
            ...
            {
                ClientId = loginStateClientId.Value,
        ...
        async Task<FinalizeLoginStatus> FinalizeLoginAsync(string nonce, string? sessionid, CancellationToken cancellationToken = default)
        {
            var r = await WaitAndRetryAsync(sleepDurations).ExecuteAsync(async () =>
            {
                ...
                if (result == null)
                {
                    throw new Exception("FinalizeLoginAsync 出现错误: result is null");
                }
    ...
    /// <inheritdoc/>
    public async Task<(bool IsSuccess, string? Message, HistoryParseResponse? History)> GetAccountHistoryDetail(SteamLoginState loginState, CancellationToken cancellationToken = default)
    {
        (bool IsSuccess, string? Message, HistoryParseResponse? History) result = (false, R.获取Steam账号消费历史记录出现错误, null);
    ...
    /// <inheritdoc/>
    public async Task<bool> GetWalletBalance(SteamLoginState loginState, CancellationToken cancellationToken = default)
    {
        if (loginState.Cookies == null)
        {
            return false;
        ...
        var r = await WaitAndRetryAsync(sleepDurations).ExecuteAsync(async () =>
        {
            ...
            if (!string.IsNullOrEmpty(html))
            {
                ...
                if (walletBalanceString != null)
                {
                    var walletBalanceSpan = walletBalanceString.AsSpan();
                    var walletBalanceSpanSplit = walletBalanceSpan.Split(' ');
                    if (walletBalanceSpanSplit.MoveNext())
                    {
                        currencySymbol = new(walletBalanceSpan[walletBalanceSpanSplit.Current]);
                    }
                }

                // 获取货币符号
                loginState.CurrencySymbol = currencySymbol;
    ...
    async Task<(SteamResult Result, PurchaseResultDetail? Detail)?> RedeemWalletCodeCore(SteamLoginState loginState, string sessionid, string walletCode, CancellationToken cancellationToken = default)
    {
        var username = loginState.Username.ThrowIsNull();
        var client = CreateClient(username);
        var container = GetCookieContainer(username);
        container.Add(loginState.Cookies!);
    ...
    /// <inheritdoc/>
    public async Task<(SteamResult Result, PurchaseResultDetail? Detail)?> RedeemWalletCode(SteamLoginState loginState, string walletCode, bool isRetry = false, CancellationToken cancellationToken = default)
    {
        string? sessionid = loginState.Cookies?["sessionid"]?.Value;
...
#pragma warning restore CA2208 // 正确实例化参数异常
        ...
        if (isRetry)
            return await WaitAndRetryAsync(sleepDurations).ExecuteAsync(async () => await RedeemWalletCodeCore(loginState, sessionid, walletCode, cancellationToken));
        else
            return await RedeemWalletCodeCore(loginState, sessionid, walletCode, cancellationToken);
    }

    /// <inheritdoc/>
    public async Task<bool> SetSteamAccountCountry(SteamLoginState loginState, string? currencyCode, CancellationToken cancellationToken = default)
    {
        if (loginState.Cookies == null)
        {
            return false;
    ...
    /// <inheritdoc/>
    public async Task<List<CurrencyData>?> GetSteamAccountCountryCodes(SteamLoginState loginState, CancellationToken cancellationToken = default)
    {
        if (loginState.Cookies == null)
        {
            throw new ArgumentNullException(nameof(loginState.Cookies),
                "Parameter Cookies not be null in SteamLoginState");
        }
...
#pragma warning restore CA2208 // 正确实例化参数异常
    ...
    /// <inheritdoc/>
    public async Task<string?> GetApiKey(SteamLoginState loginState, CancellationToken cancellationToken = default)
    {
        var username = loginState.Username.ThrowIsNull();
        var client = CreateClient(username);
        var container = GetCookieContainer(username);
    ...
    /// <inheritdoc/>
    public async Task<string?> RegisterApiKey(SteamLoginState loginState, string? domain = null, CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrEmpty(loginState.SeesionId))
        ...
        var result = await WaitAndRetryAsync(sleepDurations).ExecuteAsync(async () =>
        {
            using var stream = await SendAsync<Stream, Dictionary<string, string>>(sendArgs, data, cancellationToken);
            var parseApiKey = ParseApiKeyFromHttpContent(stream);
            if (string.IsNullOrEmpty(parseApiKey))
            {
#pragma warning disable CA2208 // 正确实例化参数异常
...
#pragma warning restore CA2208 // 正确实例化参数异常
    ...
    /// <inheritdoc/>
    public async Task<InventoryPageResponse?> GetInventories(ulong steamId, string appId, string contextId, int count = 100, string? startAssetId = null, string language = "schinese", CancellationToken cancellationToken = default)
    {
        var url = string.Format("{0}/inventory/{1}/{2}/{3}?l={4}&count={5}{6}",
            ...
            language,
            count,
            !string.IsNullOrEmpty(startAssetId) ? $"&start_assetid={startAssetId}" : null);
    ...
    /// <inheritdoc/>
    public async Task<InventoryTradeHistoryRenderPageResponse> GetInventoryTradeHistory(SteamLoginState loginState, int[]? appFilter = null, InventoryTradeHistoryRenderPageResponse.InventoryTradeHistoryCursor? cursor = null, CancellationToken cancellationToken = default)
    {
        StringBuilder urlBuilder = new($"{SteamApiUrls.STEAM_COMMUNITY_URL}/profiles/{loginState.SteamId}/inventoryhistory/?ajax=1&sessionid={loginState.SeesionId}");
    ...
    static bool TryParseDateTime([NotNullWhen(true)] string? s, CultureInfo? cultureInfo, out DateTime result)
    {
        if (s != null)
        {
            var sTrim = s.AsSpan().Trim();
            if (!DateTime.TryParse(sTrim, cultureInfo, out result))
            {
                return true;
            }
            try
            {
                if (!DateTime.TryParse(sTrim, CultureInfo.GetCultureInfo("zh-CN"), out result))
                {
                    return true;
                }
            }
            ...
            if (!DateTime.TryParseExact(sTrim, "O", CultureInfo.InvariantCulture, DateTimeStyles.None, out result))
            {
                return true;
            }
            if (!DateTime.TryParseExact(sTrim, "R", CultureInfo.InvariantCulture, DateTimeStyles.None, out result))
            {
                return true;
            }
        }
    ...
    public async IAsyncEnumerable<InventoryTradeHistoryRow> ParseInventoryTradeHistory(string html, CultureInfo? cultureInfo = null, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        Task<(DateTime date, TimeSpan timeOfDate)> ParseDateTimePart(IElement rowElement)
        {
            ...
            if (!TryParseDateTime(timeElement?.TextContent, cultureInfo, out var timeDate))
            {
                throw new ArgumentException($"日期转换失败，timeElement?.TextContent: {timeElement?.TextContent}");
            }
            ...
            if (dateElement != null)
            {
                ...
                if (!TryParseDateTime(dateText, cultureInfo, out date))
                {
                    throw new ArgumentException($"日期转换失败，dateText: {dateText}");
                }
            }
    ...
    /// <inheritdoc/>
    public async Task<IEnumerable<SendGiftHistoryItem>?> GetSendGiftHistories(SteamLoginState loginState, CancellationToken cancellationToken = default)
    {
        var username = loginState.Username.ThrowIsNull();
        var client = CreateClient(username);
        var container = GetCookieContainer(username);
        ...
        var document = await htmlParser.ParseDocumentAsync(respStream.ThrowIsNull());
...
#pragma warning restore CA2208 // 正确实例化参数异常
    ...
    /// <inheritdoc/>
    public async Task<bool> CheckAccessTokenValidation(string access_token, CancellationToken cancellationToken = default)
    {
    ...
    /// <inheritdoc/>
    public async Task<bool?> CheckAccountPhoneStatus(string access_Token, CancellationToken cancellationToken = default)
    {
        using var sendArgs = new WebApiClientSendArgs(
    ...
    /// <inheritdoc/>
    public async Task<PlayerSummariesResponse?> GetPlayerSummaries(string webApiKey, ulong[] steam64Ids, CancellationToken cancellationToken = default)
    {
        using var sendArgs = new WebApiClientSendArgs(
