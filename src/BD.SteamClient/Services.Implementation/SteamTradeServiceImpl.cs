...
public sealed class SteamTradeServiceImpl : HttpClientUseCookiesWithDynamicProxyServiceImpl, ISteamTradeService
{
    ...
    #region Tasks
    public void StartTradeTask(SteamSession steamSession, string api_key, string identity_secret, int interval, TradeTaskEnum tradeTaskEnum)
    {
        ...
        if (!_tasks.ContainsKey(taskName))
        {
            ...
            switch (tradeTaskEnum)
            {
                ...
                case TradeTaskEnum.AutoAcceptGitTrade:
                    action = async () => { await AcceptAllGiftTradeOfferAsync(steamSession, api_key, identity_secret); };
                    break;
                default:
                    return;
            }
    ...
    public async Task<bool> AcceptAllGiftTradeOfferAsync(SteamSession steamSession, string api_key, string identity_secret)
    {
        var response = await GetTradeOffersSummaryAsync(api_key);
        if (response.IsSuccessStatusCode)
        {
            ...
            {
                response = await GetTradeOffersAsync(api_key);
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (trade_offers.Count > 0)
                    {
                        foreach (var trade_offer in trade_offers)
                        {
                            ...
                            if (give_count == 0)
                            {
                                await AcceptTradeOfferAsync(steamSession, identity_secret, trade_offer.TradeOfferId, trade_offer); // 接受报价
                            }
                        }
                    }
                }
    ...
    public async Task<bool> BatchHandleTradeOfferAsync(SteamSession steamSession, string identity_secret, Dictionary<string, string> trades, bool accept)
    {
        if (!trades.Any())
            return false;

        return await new ConfirmationExecutor(identity_secret, steamSession).BatchHandleTradeOffer(trades, accept);
    }

    public async Task<bool> AcceptTradeOfferAsync(SteamSession steamSession, string identity_secret, string trade_offer_id, TradeInfo? tradeInfo)
    {
        if (tradeInfo == null)
        {
            tradeInfo = new();
        ...
        var partner = await FetchTradePartnerId(trade_offer_id, cookieContainer);
        var sessionid = await FetchSessionId(steamSession);
        var param = new Dictionary<string, string>()
        {
            { "sessionid", sessionid },
            { "stradeofferid", trade_offer_id },
            { "serverid", "1" },
            { "partner", partner },
            { "captcha", "" }
        };
        ...
        if (response.IsSuccessStatusCode)
        {
            ...
            if (root.GetProperty("response").TryGetProperty("needs_mobile_confirmation", out var _))
                return await new ConfirmationExecutor(identity_secret, steamSession).SendTradeAllowRequest(trade_offer_id);
    ...
    public async Task<bool> SendTradeOfferAsync(SteamSession steamSession, string identity_secret, List<Asset> my_itmes, List<Asset> them_items, string target_steam_id, string message)
    {
        var offer_string = CreateTradeOfferString(my_itmes, them_items);
        var sessionid = await FetchSessionId(steamSession);
        var server_id = 1;
        var param = new Dictionary<string, string>();
        ...
        var partner_account_id = Convert.ToInt32((long.Parse(target_steam_id) >> 0) & 0xFFFFFFFF);
        ...
        if (response.IsSuccessStatusCode)
        {
            ...
            var root = JsonDocument.Parse(content_string).RootElement;
            if (root.GetProperty("response").TryGetProperty("needs_mobile_confirmation", out var _))
                return await new ConfirmationExecutor(identity_secret, steamSession).SendTradeAllowRequest(root.GetProperty("response").GetProperty("tradeofferid").ToString());

            return true;
        }
        return false;

        string CreateTradeOfferString(List<Asset> my_itmes, List<Asset> them_items)
        {
            var offer = new
            {
                Newversion = true,
            ...
            return JsonSerializer.Serialize(offer, options: new JsonSerializerOptions() { PropertyNameCaseInsensitive = true });
        }
    }
    ...
    private static async Task<string> FetchTradePartnerId(string trade_offer_id, CookieContainer cookie)
    {
        ...
        var page_text = await client.GetStringAsync(url);
    ...
    private static async Task<string> FetchSessionId(SteamSession steamSession)
    {
        ...
        return steamSession.CookieContainer.GetCookieValue(new Uri(STEAM_COMMUNITY_URL), "sesssionid")!;
    }
    #endregion

    internal class ConfirmationExecutor
    {
        private string _identitySecret;

        private SteamSession _steamSession;

        public ConfirmationExecutor(string identitySecret, SteamSession steamSession)
        {
        ...
        private Dictionary<string, string> CreateConfirmationParams(string tag)
        {
            ...
            var timehash = CteateTimeHash(_identitySecret, tag, timestamp);
