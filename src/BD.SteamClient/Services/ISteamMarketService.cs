...
public interface ISteamMarketService
{
    ...
    /// <returns></returns>
    Task<MarketItemPriceOverviewResponse> GetMarketItemPriceOverview(string appId, string marketHashName, int currency = 1);
    ...
    /// <returns></returns>
    Task<MarketItemOrdersHistogramResponse> GetMarketItemOrdersHistogram(long marketItemNameId, string country = "CN", int currency = 23, string language = "schinese");
    ...
    /// <returns></returns>
    Task<SellItemToMarketResponse> SellItemToMarket(SteamLoginState loginState, string appId, string contextId, long assetId, int amount, int price);
