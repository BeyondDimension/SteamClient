...
public class SteamAppViewModel : ReactiveObject
...
#endif
{
    ...
    const string NodeAppType = "type";

    const string NodeCommon = "common";

    const string NodeConfig = "config";

    const string NodeExtended = "extended";

    const string NodeId = "gameid";

    const string NodeName = "name";

    const string NodeParentId = "parent";

    const string NodePlatforms = "oslist";

    const string NodePlatformsLinux = "linux";

    const string NodePlatformsMac = "mac";

    const string NodePlatformsWindows = "windows";

    const string NodeSortAs = "sortas";

    const string NodeDeveloper = "developer";

    const string NodePublisher = "publisher";

    const string NodeLaunch = "launch";

    public SteamAppViewModel() { }

    public SteamAppViewModel(uint appid)
    {
        AppId = appid;
    }
    ...
    public SteamAppViewModel ExtractReaderProperty(SteamAppPropertyTable properties, uint[]? installedAppIds = null)
    {
        if (properties != null)
        {
    ...
    public static SteamAppViewModel? FromReader(BinaryReader reader, uint[]? installedAppIds = null, bool isSaveProperties = false, bool is0x07564428 = true)
    {
        ...
        {
            Log.Error(nameof(SteamAppViewModel), ex, string.Format("Failed to load entry with appId {0}", app.AppId));
        }
        return app;
    }
    ...
    /// <summary>
    /// <see cref="SteamApp"/> 隐式转换 <see cref="SteamAppViewModel"/>
    /// </summary>
    /// <param name="steamApp"></param>
    public static implicit operator SteamAppViewModel(SteamApp steamApp) => new(steamApp);

    /// <summary>
    /// <see cref="SteamApp"/> 构造实例 <see cref="SteamAppViewModel"/>
    /// </summary>
    /// <param name="steamApp"></param>
    public SteamAppViewModel(SteamApp steamApp)
    {
        Model = steamApp;
    }
}
