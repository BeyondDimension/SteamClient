...
sealed partial class Base32
{
    ...
    /// <returns></returns>
    /// <exception cref="AuthenticatorBase32DecodingException"></exception>
    public byte[] Decode(string encoded)
    {
        ...
        foreach (var c in encoded.ToCharArray())
        {
            if (_map.ContainsKey(c) == false)
                throw new AuthenticatorBase32DecodingException("Illegal character: " + c);
            buffer <<= _shift;
            buffer |= _map[c] & _mask;
            bitsLeft += _shift;
            if (bitsLeft >= 8)
