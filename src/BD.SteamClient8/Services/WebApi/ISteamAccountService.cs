...
public interface ISteamAccountService
{
    ...
    /// <returns></returns>
    Task<ApiRspImpl> DoLoginV2Async(SteamLoginState loginState);
    ...
    /// 获取 Steam 账号消费历史记录
    /// </summary>
    Task<ApiRspImpl<(bool IsSuccess, string? Message, HistoryParseResponse? History)>> GetAccountHistoryDetail(SteamLoginState loginState);

    /// <summary>
    /// 获取 Steam 账号余额货币类型及账号区域信息，并检测账号是否定区
    /// </summary>
    Task<ApiRspImpl<bool>> GetWalletBalance(SteamLoginState loginState);
    ...
    /// 设置 Steam 账号区域到指定位置
    /// </summary>
    Task<ApiRspImpl<bool>> SetSteamAccountCountry(SteamLoginState loginState, string currencyCode);

    /// <summary>
    /// 获取当前可设置的区域列表
    /// </summary>
    Task<ApiRspImpl<List<CurrencyData>?>> GetSteamAccountCountryCodes(SteamLoginState loginState);
    ...
    /// <returns></returns>
    Task<ApiRspImpl<string?>> RegisterApiKey(SteamLoginState steamLoginState, string? domain = null);
    ...
    /// <returns></returns>
    Task<ApiRspImpl<bool>> CheckAccessTokenValidation(string accesstoken);
}
